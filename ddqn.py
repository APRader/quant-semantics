# Code adapted from https://github.com/fschur/DDQN-with-PyTorch-for-OpenAI-Gym
...
def evaluate(Qmodel, env, repeats, device):
    ...
    Qmodel.train()
    return rewards / repeats, performances / repeats
...
def main(env, gamma=0.99, lr=1e-3, min_episodes=20, eps=1, eps_decay=0.995, eps_min=0.01, update_step=10, batch_size=64,
         update_repeats=50,
         num_episodes=3000, seed=42, max_memory_size=50000, lr_gamma=0.9, lr_step=20, measure_step=100,
         measure_repeats=100, hidden_dim=64, horizon=np.inf, render=True,
         render_step=50):
    """
    ...
    for episode in range(num_episodes):
        ...
        if episode % measure_step == 0:
            ...
            if episode == 0:
                toc = tic
            reward, performance = evaluate(Q_1, env, measure_repeats, device)
            rewards.append(reward)
            performances.append(performance)
            # ([episode, evaluate(Q_1, env, MEASURE_REPEATS, device)])
            print("\tEpisode", episode)
            print(f"\t\tTraining time: {tic - toc:0.2f}")
            toc = time.perf_counter()
            print(f"\t\tEvaluation time: {toc - tic:0.2f}")
            print("\t\trewards: ", rewards[-1])
            print("\t\tperformance: ", performances[-1])
