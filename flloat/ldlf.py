...
class LDLfPropositionalAtom(AtomicFormula, LDLfFormula):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        """Quantitative semantics for formula."""
        return self.to_nnf().value(i, pos)
...
class LDLfAnd(LDLfBinaryOperator):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        """Quantitative semantics for formula."""
        return min(f.value(i, pos) for f in self.formulas)
...
class LDLfOr(LDLfBinaryOperator):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        """Quantitative semantics for formula."""
        return max(f.value(i, pos) for f in self.formulas)
...
class LDLfDiamond(LDLfTemporalFormula):
    ...
    def value(self, tr: FiniteTrace, pos: int = 0):
        """Quantitative semantics for formula."""
        # last + 1 in order to include the last step
        return max(
            [min(self.r.value(tr, pos, j), self.f.value(tr, j))
            for j in range(pos, len(tr) + 1)], default=0
        )
...
class LDLfBox(LDLfTemporalFormula):
    ...
    def value(self, tr: FiniteTrace, pos: int = 0):
        ...
        # last + 1 in order to include the last step
        return min(
            [max(1 - self.r.value(tr, pos, j), self.f.value(tr, j))
            for j in range(pos, len(tr) + 1)], default=1
        )
...
class RegExpPropositional(RegExpFormula):
    ...
    def value(self, tr: FiniteTrace, start: int = 0, end: int = 0):
        """Quantitative semantics for formula."""
        return (
            self.pl_formula.value(tr[start]) if end == start + 1 and 0 <= start < len(tr) else 0
        )
...
class RegExpTest(REfUnaryOperator[LDLfFormula]):
    ...
    def value(self, tr: FiniteTrace, start: int = 0, end: int = 0):
        """Quantitative semantics for formula."""
        return self.f.value(tr, start) if start == end else 0
...
class RegExpUnion(REfBinaryOperator):
    ...
    def value(self, tr: FiniteTrace, start: int = 0, end: int = 0):
        """Quantitative semantics for formula."""
        return max(f.value(tr, start, end) for f in self.formulas)
...
class RegExpSequence(REfBinaryOperator):
    ...
    def value(self, tr: FiniteTrace, start: int = 0, end: int = 0):
        ...
        return max(
            [min(f1.value(tr, start, k), f2.value(tr, k, end))
            for k in range(start, end + 1)], default=0
        )
...
class RegExpStar(REfUnaryOperator):
    ...
    def value(self, tr: FiniteTrace, start: int = 0, end: int = 0):
        """Quantitative semantics for formula."""
        return max(
            [min(self.f.value(tr, start, k), self.value(tr, k, end))
            for k in range(start + 1, end + 1)], default=0
        ) if start != end else 1
