...
class LTLfAtomic(AtomicFormula, LTLfFormula):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        ...
        if len(i) > 0:
            return PLAtomic(self.s).value(i[pos])
        else:
            return 0
...
class LTLfNext(LTLfUnaryOperator):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        """Quantitative semantics for formula."""
        return self.f.value(i, pos + 1) if pos < len(i) - 1 else 0
...
class LTLfWeakNext(LTLfUnaryOperator):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        """Quantitative semantics for formula."""
        return self.f.value(i, pos + 1) if pos < len(i) - 1 else 1
...
class LTLfUntil(LTLfBinaryOperator):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        ...
        return max(
            [min(f2.value(i, j), min([f1.value(i, k) for k in range(pos, j)], default=1))
            for j in range(pos, len(i))], default=0
        )
...
class LTLfRelease(LTLfBinaryOperator):
    ...
    def value(self, i: FiniteTrace, pos: int = 0):
        ...
        return min(
            [max(f2.value(i, j), max([f1.value(i, k) for k in range(pos, j)], default=0))
            for j in range(pos, len(i))], default=1
        )
